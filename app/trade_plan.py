...
def _classify_setup_class(score_row: dict, feature_row: dict, setup_evolution: str) -> str:
    ...
    if total_score >= 65 and pullback_score <= 12 and depth >= Decimal("-3"):
...
def _classify_plan_status(
    ...
    if setup_evolution == "RESPONDING":
        if total_score >= 65 and trend_score >= 25 and rr_hypothesis == "RR_GOOD" and "thin_response" not in warnings:
            return "READY_PLAN"
        return "WATCH_PLAN"
    if setup_evolution == "BREAKING_OUT":
        if total_score >= 65 and trend_score >= 25 and rr_hypothesis == "RR_GOOD":
            return "READY_PLAN"
...
def _warnings(score_row: dict, feature_row: dict, observations: _EvolutionObservations) -> list[str]:
    ...
    if total_score >= 80 and pullback_score <= 12:
